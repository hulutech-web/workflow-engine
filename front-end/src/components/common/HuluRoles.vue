...
function selectRowChange(row: RowVO) {
  addTitleHandle(row)
}
...
<template>
  <div>
    <vxe-toolbar>
      <template #buttons>
        <vxe-button status="primary" @click="insertEvent()">
          新增
        </vxe-button>
        <vxe-button status="danger" @click="removeSelectEvent()">
          删除
        </vxe-button>
      </template>
    </vxe-toolbar>
      ...
      <vxe-column field="rule_name" title="规则类型" :edit-render="{}">
        <template #edit="{ row }">
          <vxe-select v-model="row.rule_name" clearable transfer @change="selectRowChange(row)">
            <vxe-option
              v-for="item in roleList" :key="item.label" :value="item.value"
              :label="item.label"
            />
          </vxe-select>
        </template>
