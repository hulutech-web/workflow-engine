...
export default () => {
  ...
  const storeFlowtype = async (data) => {
    return await request.Post(`flowtype`,data);
  };
  const deleteFlowtype = async (id) => {
    return await request.Delete(`flowtype/${id}`);
  };

  const showFlowtype = async (id) => {
    return await request.Get(`flowtype/${id}`);
  };
  const updateFlowtype = async (data) => {
    return await request.Put(`flowtype/${data.id}`,data);
  };
      ...
      ajax: {
        ...
        query: ({page, sorts, filters, form}) => {
          return new Promise(async (resolve, reject) => {
            ...
            const {data} = await request.Get(`flowtype?pageSize=${page.pageSize}&currentPage=${page.currentPage
