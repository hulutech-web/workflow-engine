...
function handleLogin() {
  formRef.value?.validate(async (errors) => {
    ...
    const { account, pwd, tenantId } = formValue.value

    if (isRemember.value)
      local.set('loginAccount', { account, pwd, tenantId })
    else local.remove('loginAccount')

    await authStore.login(account, pwd, tenantId)
  ...
  <div>
    ...
    <n-form ref="formRef" :rules="rules" :model="formValue" :show-label="false" size="large">
      <n-form-item path="tenantId">
        <n-select v-model:value="formValue.tenantId" :options="authStore.tenant" :placeholder="$t('login.tenantPlaceholder')" />
      </n-form-item>
