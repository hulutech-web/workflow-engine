...
import (
	...
	"github.com/hulutech-web/workflow-engine/app/api/service/workflow/official_plugin"
...
func (w *Engine) SetFirstProcessAuditor(entry models.Entry, flowlink models.Flowlink) error {
	return w.db.Transaction(func(tx *gorm.DB) error {
		...
		if myFlowlink.ID == 0 {
			...
			proc.Concurrence = carbon.NewTimestamp(carbon.Now())
		...
		for _, emp := range auditors_emps {
			...
			proc2.Concurrence = carbon.NewTimestamp(carbon.Now())
...
func (w *Engine) GoToProcess(entry models.Entry, processID int) error {
	...
	//current_time := carbon.NewTimestamp(carbon.Now())
	processName := ""
	...
	for _, auditor := range auditors {
		w.db.Model(&models.Proc{}).Create(&models.Proc{
			...
			Status:      0,
			IsRead:      0,
			Concurrence: carbon.NewTimestamp(carbon.Now()),
		})
	}
	return nil
}
...
func (w *Engine) Transfer(process_id int, user models.Emp, content string) error {
	...
	if fkcount > 1 {
		...
		curr_time := carbon.NewTimestamp(carbon.Now())
		for _, auditor := range auditors {
...
func (w *Engine) UnPass(proc_id int, user models.Emp, content string) {
	...
	todoProc.Concurrence = carbon.NewTimestamp(carbon.Now())
