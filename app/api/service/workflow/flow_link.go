...
type FlowlinkService interface {
	Update(ctx *gin.Context, flow models.Flow) error
}

type flowlinkService struct {
	db *gorm.DB
}

func (f *flowlinkService) Update(ctx *gin.Context, flow models.Flow) error {
	if flow.Jsplumb == "" {
		return errors.New("流程图数据为空")
	}
	//解析流程图数据
	jsMap := common.Plumb{}
	err := json.Unmarshal([]byte(flow.Jsplumb), &jsMap)
	if err != nil {
		return errors.New("解析流程图数据错误")
	}
	tx := f.db.Begin()
	//更新总数
...
func NewFlowlinkService(db *gorm.DB) FlowlinkService {
	return &flowlinkService{db: db}
}
